...
export class TransferenciasController {
  ...
  @ApiOperation({
    summary:
      'Empresas y centros disponibles para transferencia (sin params de ruta)',
  })
  ...
  @ApiOperation({
    summary:
      'Opciones de transferencia paginadas (empresas + centros) con búsqueda',
  })
  @ApiResponse({ status: 200, description: 'OK' })
  ...
  async getConteosPorCentros(
    ...
    const t0 = Date.now();
    const res = await this.trabajadoresService.contarTrabajadoresPorCentros(
      userId,
      centroIds,
    );
    const t = Date.now() - t0;
