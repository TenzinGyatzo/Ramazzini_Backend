...
describe('ExpedientesController — removeDocument (H-08)', () => {
  ...
  it('pasa el userId del JWT al servicio', async () => {
    ...
    const controller = new ExpedientesController({
      removeDocument,
    } as unknown as ExpedientesService);

    await controller.removeDocument('notaMedica', '507f1f77bcf86cd799439011', {
      userId: 'actor-123',
    } as Parameters<ExpedientesController['removeDocument']>[2]);
